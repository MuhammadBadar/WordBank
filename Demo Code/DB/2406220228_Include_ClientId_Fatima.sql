...
CREATE TABLE `acc_chartofaccount`  (
  ...
  PRIMARY KEY (`ClientId`, `Id`) USING BTREE,
...
/*update*/
 ...
 then
    ...
    where Acc_ChartOfAccount.ClientId =prm_clientId AND Acc_ChartOfAccount.Id =prm_Id ;
end if;
IF prm_dBoperation = 'Delete'
    then
            delete from Acc_ChartOfAccount
		where
			 Acc_ChartOfAccount.ClientId=prm_clientId AND  Acc_ChartOfAccount.Id =prm_Id;
END if;
...
BEGIN
			...
			left join enumline as type on (type.Id= acc.CoaTypeId and type.ClientId=0)
			 
			 left join enumline as level on (level.Id= acc.CoaLevelId and level.ClientId=0)
			 
			 left join ACC_ChartOfAccount parentAcc on 
			 (parentAcc.Id=acc.ParentCoaId and parentAcc.ClientId=acc.ClientId)
			";
